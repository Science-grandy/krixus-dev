	          ...
	          <div class="col-md-2">
	          	<div class="form-group">
	          		...
	          		<?php } ?>
	          			...
	          			elseif (isset($s_id)) {
	          			 	$query = $conn->query("SELECT DISTINCT c.id, c.level, c.section FROM classes c INNER JOIN students s ON s.class_id = c.id WHERE s.id =".$s_id);
	          			 } else {
...
<script>
	...
	function calc_ave() {
	    var total = 0;
	    var className = $('#classSelect').text().trim().split('-')[0];
	    var i;
	    ...
	    $('#total-list [name="total[]"]').each(function() {
	        total += parseFloat($(this).val()) || 0;  // Use `|| 0` to avoid NaN
	    });

	    var average = parseFloat(total / i).toLocaleString('en-US', {style: 'decimal', maximumFractionDigits: 2});
	    $('#average').text(average);
	    $('[name="student_average"]').val(average);
	}
